...
var kanban_controller_default = class extends Controller {
  ...
  async moveCard(cardId, columnId, position) {
    const boardId = this.element.querySelector("[data-board-id]")?.dataset.boardId || window.location.pathname.match(/\/boards\/(\d+)/)?.[1];
    if (!boardId) return;
    ...
    try {
      ...
      console.error("Error moving card:", error2);
    }
  }
  showNewColumnForm() {
    ...
    if (cardId && boardId) {
      ...
      if (drawer && content) {
        ...
        fetch(`/boards/${boardId}/cards/${cardId}`, {
          ...
          console.error("Error loading card:", error2);
